...
import { getTrackedFiles } from './fs-tracker.js';
import { validateAllCliArguments } from './cli-validation.js';
import { getNonInteractiveRequiredFields } from './prompts.js';
 ...
 * @returns {boolean}
 */
export function isDryRunMode(options) {
  return Boolean(options.dryRunInteractive || options.dryRunNonInteractive);
}
 ...
 * @param {object} options - Generator options
 */
export function initializeDryRunMode(options) {
  const dryRunState = isDryRunMode(options);
  store.setDryRun(dryRunState);
}
 ...
 */
export function normalizeDryRunOptions(options) {
 ...
 * @returns {boolean}
 */
export function shouldSkipValidation(options) {
 ...
 */
export function computePropsForDryRun(generator, allFields) {
 ...
 */
export function handleDryRunPrompting(generator, allFields) {
 ...
 * @returns {string} - Source description
 */
export function getOptionSource(optionName, options, allFields) {
 ...
 * @param {object} allFields - Field definitions
 */
export function showTrackedFiles(generator, allFields) {
  console.log(chalk.cyan('\nDRY RUN - Configuration'));
  ...
  console.log(chalk.yellow('Files to be created/modified:'));
  ...
  if (trackedFiles.length > 0) {
    console.log(
      chalk.blue(`Files that would be created/modified: ${trackedFiles.length}`),
    );
  }
